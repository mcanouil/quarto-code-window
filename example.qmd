...
```{.python filename="custom.py" code-window-lines="2-3"}
def greet(name):
...
Set `code-window-collapse` to wrap a code block in a `<details>` element.
The title bar becomes the `<summary>`.
This feature is HTML-only; Typst and PDF output is unaffected.

### Closed by Default

```{.python filename="hidden.py" code-window-collapse="closed"}
print("Click the title bar to expand.")
print("Useful for long boilerplate.")
```

### Open by Default

```{.python filename="visible.py" code-window-collapse="open"}
print("Visible by default, can be collapsed.")
```
